...
var (
	...
	ftsKeywordAsPhrase = regexp.MustCompile(`(?i)\b(AND|OR|NOT|NEAR)\b`)
)

// escapeFTSKeyword 转义 SQLite FTS 查询关键字中的特殊字符。包括：
//   - 将 fts5 布尔操作关键字 AND, OR, NOT, NEAR（不区分大小写）用双引号包裹为短语，
//     使它们作为普通单词参与查询，而不是被解释为操作符。
...
func escapeFTSKeyword(s string) string {
	...
	result := ftsKeywordAsPhrase.ReplaceAllStringFunc(b.String(), func(match string) string {
		return `"` + match + `"`
	})
